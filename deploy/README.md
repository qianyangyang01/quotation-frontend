...
应用发布对 PostgreSQL、Redis、MinIO 使用 `pull --policy missing`：复用本机已经存在的配置版本，只有镜像缺失时才拉取，拉取失败仍会阻止发布。数据服务升级应单独更改并验证镜像版本，不随应用发布刷新相同标签。这样已有生产实例不会因为镜像仓库入口暂时不可用而被迫更换存储版本。集成测试使用 MinIO 官方 Quay 仓库的同一固定版本。
