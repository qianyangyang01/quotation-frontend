...
const country = ref(''), query = ref(''), countrySearch = ref(''), provider = ref(''), page = ref(1), size = ref(10)
...
const allChecked = computed(() => filtered.value.length > 0 && filtered.value.every(row => selected.value.includes(row.key)))
watch(country, () => { selected.value = []; editing.value = false; query.value = ''; provider.value = ''; page.value = 1; error.value = ''; message.value = '' })
watch([query, provider, size], () => { page.value = 1 })
...
function view(row: typeof allRows.value[number]) {
  ...
  const displaySettings = { ...props.modelValue, countries: props.modelValue.countries.filter(item => item.country === country.value) }
...
function toggleFiltered() { const keys = new Set(filtered.value.map(row => row.key)); selected.value = allChecked.value ? selected.value.filter(key => !keys.has(key)) : [...new Set([...selected.value, ...keys])] }
function edit(key?: string) {
  if (key) {
    selected.value = [key]
    const rule = ruleFor(key), row = allRows.value.find(item => item.key === key)!
    draft.value = rule ? { ...rule } : isChc(key, row.carrier) ? { mode: 'weight', amount: 0.6, currency: 'EUR', perKg: 1.5 } : { mode: 'fixed-order', amount: 0.3, currency: 'USD', perKg: 0 }
  }
...
function apply() {
  try {
    ...
    const rules = new Map((props.modelValue.countries.find(row => row.country === country.value)?.channelRules || []).map(row => [row.key, row]))
    for (const row of allRows.value.filter(row => keys.has(row.key))) {
      const rule = isChc(row.key, row.carrier) ? { key: row.key, mode: 'weight' as const, currency: 'EUR' as const, amount: 0.6, perKg: 1.5 } : { ...draft.value, key: row.key }
      if (isChc(row.key, row.carrier)) kept++
      validateChannelTaxRule(rule); rules.set(row.key, rule)
    }
    update({ ...props.modelValue, countries: props.modelValue.countries.map(row => row.country === country.value ? { ...row, enabled: true, selected: true, channelRules: [...rules.values()] } : row) })
...
<template>
  <section class="channel-tax-workspace" aria-label="渠道税费设置">
    <header class="workspace-title"><div><h2>渠道税费设置</h2><p>按国家维护，支持批量设置与单独调整</p></div><div class="actions"><input ref="importInput" hidden type="file" accept=".xlsx" @change="importFile"><button :disabled="saving || importing" @click="importInput?.click()">{{ importing ? '正在读取…' : '导入表格' }}</button><button class="primary" :disabled="saving || importing" @click="emit('save')">{{ saving ? '保存中…' : '保存并发布' }}</button></div></header>
    ...
    <div class="layout" :class="{ editing }">
      ...
      <main class="matrix"><header><h3>{{ label(country) || '请选择国家' }}</h3><p>同一项关税只计入一次；历史报价保持原金额</p></header><div v-if="country" class="filters"><input v-model="query" placeholder="搜索物流商或渠道" aria-label="搜索税费渠道"><select v-model="provider" aria-label="筛选税费物流商"><option value="">全部物流商</option><option v-for="name in providers" :key="name">{{ name }}</option></select><button :disabled="!selected.length || saving" @click="edit()">批量设置</button></div>
        <div v-if="country" class="selection"><button :disabled="!allRows.length || saving" @click="selected = allRows.map(row => row.key)">全选全部渠道（{{ allRows.length }}）</button><span>跨分页生效 · 已选 {{ selected.length }} 个</span><button v-if="selected.length" @click="selected = []; editing = false">取消选择</button></div>
        <div class="table-scroll"><table><thead><tr><th><input type="checkbox" aria-label="全选筛选结果" :checked="allChecked" :disabled="!filtered.length || saving" @change="toggleFiltered"></th><th>物流商 · 渠道</th><th>收费方式</th><th>原币金额</th><th>折合美元</th><th>操作</th></tr></thead><tbody><tr v-for="row in visible" :key="row.key"><td><input type="checkbox" :aria-label="`选择${row.carrier}${row.channel}`" :checked="selected.includes(row.key)" :disabled="saving" @change="toggle(row.key)"></td><td><b>{{ row.carrier }}</b><small>{{ row.channel }}</small></td><td><span class="badge" :class="view(row).mode">{{ view(row).name }}</span></td><td>{{ view(row).amount }}</td><td>{{ view(row).usd }}</td><td><button class="text-button" :disabled="saving" :aria-label="`编辑${row.carrier}${row.channel}税费`" @click="edit(row.key)">编辑</button></td></tr><tr v-if="!visible.length"><td colspan="6" class="empty">{{ country ? '当前没有匹配的启用渠道' : '添加国家后开始设置渠道税费' }}</td></tr></tbody></table></div>
        <footer><small>1 USD = {{ exchange.usdCny.toFixed(4) }} CNY · 最近保存：{{ modelValue.updatedAt }}</small><div><select v-model.number="size" aria-label="每页渠道数"><option :value="10">10 条</option><option :value="30">30 条</option><option :value="50">50 条</option></select><button :disabled="page <= 1" @click="page--">‹</button><span>{{ page }} / {{ pages }}</span><button :disabled="page >= pages" @click="page++">›</button></div></footer>
      </main>
      <aside v-if="editing" class="editor"><header><h3>{{ selected.length === 1 ? '编辑渠道税费' : '批量设置关税' }}</h3><button aria-label="关闭税费编辑" @click="editing = false">×</button></header><p>{{ label(country) }} · 已选 {{ selected.length }} 个渠道</p><label>收费方式<select v-model="draft.mode" aria-label="收费方式"><option value="fixed-order">固定金额 / 单</option><option value="weight">按整单重量</option><option value="exempt">已含税（含）</option><option value="no-tax">不加收（无）</option><option value="unavailable">该目的地不支持（不发）</option></select></label><template v-if="draft.mode === 'fixed-order' || draft.mode === 'weight'"><label>{{ draft.mode === 'weight' ? '每单固定费用' : '原币金额' }}<div class="amount"><input :value="amountInput" @input="inputAmount" @blur="finishAmount" aria-label="原币金额" type="number" min="0" max="1000000" step="0.01"><select v-model="draft.currency" aria-label="费用币种"><option>USD</option><option>CNY</option><option>EUR</option></select></div></label><label v-if="draft.mode === 'weight'">每公斤费用<input v-model.number="draft.perKg" aria-label="每公斤费用" type="number" min="0" step="0.01"></label><div class="calculation"><small>{{ draft.mode === 'weight' ? '含包材计费重 1kg 示例' : '折合美元' }}</small><strong>{{ preview }}</strong><small>按财务汇率自动换算</small></div></template><p class="notice">更新仅影响当前国家所选渠道的新报价。欧盟 CHC 三条渠道保留专用欧元重量规则。</p><div class="actions"><button @click="editing = false">取消</button><button class="primary" :disabled="saving || !selected.length" @click="apply">应用到所选渠道</button></div></aside>
    </div>
  </section>
</template>
