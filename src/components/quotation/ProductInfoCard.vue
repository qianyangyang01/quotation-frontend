...
<template>
  <section class="product-card">
    ...
    <div class="info"><small>当前报价商品 <em v-if="product.status==='采购资料已加载'" :class="{legacy:product.purchaseDataSource==='legacy_2026'}">{{ product.purchaseDataSource==='legacy_2026' ? '2026旧数据' : '新数据' }}</em></small><h2>{{ product.name }}</h2><p><code>{{ product.sku }}</code><span>采购：{{ product.supplier }}</span><span class="stock" :class="{ out:product.stockStatus==='无货', pending:product.stockStatus==='待确认' }">是否有货：{{ product.stockStatus }}</span></p></div>
...
.info>small em{margin-left:6px;padding:3px 7px;border-radius:9px;background:#e7f6ed;color:#17794f;font-size:8px;font-style:normal;font-weight:800}.info>small em.legacy{background:#eee8ff;color:#5f42a8}
</style>
