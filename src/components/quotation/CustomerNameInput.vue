...
const active = ref(-1)
// Restored/free-text names are values, not a request to filter the browse list.
const query = ref('')
const matches = computed(() => props.customers.filter(row => row.enabled && row.name.toLocaleLowerCase().includes(query.value.trim().toLocaleLowerCase())))
function close() { open.value = false; query.value = ''; active.value = -1 }
function browse() { const show = !open.value || Boolean(query.value); close(); open.value = show }
function input(value: string) { query.value = value; emit('update:modelValue', value); open.value = true; active.value = -1 }
...
<template>
  <div class="customer-picker" @focusout="close">
    ...
    <div v-if="open" id="finance-customer-options" role="listbox" class="options">
      <button v-for="(row,index) in matches" :key="row.id" type="button" role="option" :aria-selected="row.id === selectedId" :class="{ active: index === active }" @mousedown.prevent @click="choose(row.id)">{{ row.name }}<small>{{ operationFeesLabel(row) }}</small></button>
...
.options button{align-items:flex-start;flex-direction:column;gap:4px}.options small{white-space:normal;line-height:1.6}
</style>
