      ...
      <section v-for="group in groups" :key="group.country" :class="{ open:expandedCountries.has(group.country) }">
        ...
        <template v-if="expandedCountries.has(group.country)">
          ...
          <article v-for="row in group.rows" :key="rowKey(row)" :class="{ primary:isPrimary(row) }">
            <span><span class="channel-name-line"><b>{{ row.carrier }}｜{{ row.transport }}</b><QuoteTaxMeta v-if="row.available !== false" :row="row" /></span><small v-for="(message, quantity) in row.quantityMessages" :key="quantity">{{ message }}</small><small>渠道编码：{{ row.channelCode || '—' }} · 计费规则：{{ row.rule }}<template v-if="row.quoteRegion"> · {{ row.quoteRegion }}</template></small><em v-if="isPrimary(row)">整单首选</em></span><strong>{{ row.eta }}</strong>
            <span><small v-if="row.available === false">{{ row.availabilityMessage }}</small><b>{{ formatUsd(row.quote1) }}</b><small>{{ formatCny(row.quote1) }}</small></span><span><b>{{ formatUsd(row.quote2) }}</b><small>{{ formatCny(row.quote2) }}</small></span><span><b>{{ formatUsd(row.quote3) }}</b><small>{{ formatCny(row.quote3) }}</small></span><span class="custom"><b>{{ formatUsd(row.quoteCustom) }}</b><small>{{ formatCny(row.quoteCustom) }}</small></span>
          </article>
        </template>
      </section>
    </div>
