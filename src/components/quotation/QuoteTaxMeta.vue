...
<template>
  <span class="fee-metadata"><mark class="tax-channel-badge" :class="{ exempt:row.taxIncluded, missing }">{{ badgeText }}</mark><mark v-if="row.surchargeEnabled" class="tax-channel-badge" :class="{ exempt: row.surchargeExempt }">{{ row.surchargeExempt ? '免附加费' : `国家附加费 ${Number(row.surchargeUsd || 0).toFixed(2)}/单` }}</mark></span>
</template>
