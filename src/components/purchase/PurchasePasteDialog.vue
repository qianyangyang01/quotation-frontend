...
async function copySaved() {
  ...
  try {
    ...
    message.value = `已复制上次成功新增的${savedCount.value}条SKU和品类，可直接粘贴给业务员`
  } catch {
  ...
  <Teleport to="body">
    <div class="paste-overlay" @keydown.esc.prevent="close">
      <section class="paste-dialog" role="dialog" aria-modal="true" aria-labelledby="paste-title">
        ...
        <p class="paste-help">必填：正式SKU、克重、起订量、基准采购单价、票点（无票点请填0%）；未包邮时需1件及10件总运费。其他列可留空，长宽高、阶梯价填写时须完整。保存以本页校验为准。</p>
        ...
        <footer><span>已填写 {{ check.records.length }} 条<small>{{ savedCount ? `上次成功新增 ${savedCount} 条，可复制SKU和品类。` : '空行不保存；新增时遇到已有SKU会提示，不覆盖原商品。' }}</small></span><div><button :disabled="busy" @click="close">{{ savedCount && !check.records.length ? '完成' : '取消' }}</button><button class="primary" :disabled="busy || !check.canSave" @click="save">{{ busy ? '正在保存…' : `保存新增 ${check.records.length} 条` }}</button><button class="copy-saved" :disabled="busy || copying || !savedShareText" @click="copySaved">{{ copying ? '正在复制…' : '一键复制SKU和品类' }}</button></div></footer>
