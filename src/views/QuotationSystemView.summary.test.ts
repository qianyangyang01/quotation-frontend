...
function execute(options: ReturnType<typeof option>[], price: unknown = { quoteUsd: 19.75, quoteCny: 132.33, cost: 108.74 }) {
  const calls: unknown[][] = []
  const run = new Function('logisticsRuleForChannel', 'billingQuoteRegion', 'savedQuoteRows', 'products', 'quantityCostBreakdown', js + ';return selectedQuoteSummary')(() => ({}), (_rule: unknown, _country: string, region: string) => region, { value: options }, { value: [{ country: '美国' }] }, (...args: unknown[]) => { calls.push(args); return price })
