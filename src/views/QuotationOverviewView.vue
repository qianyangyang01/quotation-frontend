  ...
  <div class="overview-app">
    <main>
      ...
      <section class="card detail-card">
        ...
        <div class="table-scroll"><table><thead><tr><th>报价编号 / 时间</th><th>客户</th><th>业务员</th><th>国家</th><th>主 SKU / 品类</th><th>报价(USD)</th><th>报价(RMB)</th><th>操作</th></tr></thead><tbody>
          <tr v-for="row in pagedDetails" :key="row.id"><td><b>{{ row.no }}</b><small>{{ dateTime(row.createdAt) }}</small></td><td>{{ row.customerName }}</td><td><b>{{ row.salespersonName }}</b><small>{{ row.salespersonAccount }}</small></td><td>{{ recordCountries(row).join('、') || '—' }}</td><td><b>{{ quotationSkus(row).join('、') || '—' }}</b><small>{{ categoryFor(row) }}</small></td><td>{{ moneyUsd(row.systemQuoteUsd) }}</td><td>{{ moneyCny(row.systemQuoteCny) }}</td><td><RouterLink :to="{path:'/quotation/records',query:{record:row.id}}">查看详情</RouterLink></td></tr>
          <tr v-if="!pagedDetails.length"><td colspan="8" class="empty">没有找到匹配的报价明细</td></tr>
        </tbody></table></div>
