...
const financeSurchargeSettings = ref<FinanceSurchargeSettings>({ countries: [], providers: [], updatedAt: '' })
...
const financeSurchargeCountries = computed(() => {
  const query = financeSurchargeCountrySearch.value.trim().toLowerCase()
  return financeSurchargeSettings.value.countries
...
const savingGradeSettings = ref(false)
async function saveGradeSettings() {
  if (savingGradeSettings.value) return
  if (customerGradeSettings.value.some(setting => !Number.isFinite(setting.coefficient) || setting.coefficient <= 0)) {
    toast('客户等级系数必须为大于 0 的有效数字')
    return
  }
  savingGradeSettings.value = true
  try {
    customerGradeSettings.value = await saveCustomerGradeSettings(customerGradeSettings.value)
    toast('客户等级计算系数已保存')
  } catch (error) { toast(error instanceof Error ? error.message : '客户等级系数保存失败，请重试') }
  finally { savingGradeSettings.value = false }
}
...
function financeCarrierFullySelected(index: number) {
  const channels = financeChannelsForSelectedCarrier(index)
  const selected = new Set(financePolicyForm.value.countryRules[index].allowedChannels)
  return channels.length > 0 && channels.every(channel => selected.has(channel.key))
}
function toggleFinanceCarrierChannels(index: number) {
  const rule = financePolicyForm.value.countryRules[index]
  rule.allowedChannels = toggleFinanceChannelSelection(rule.allowedChannels, financeChannelsForSelectedCarrier(index).map(channel => channel.key))
}
  ...
  <div class="module-app">
    ...
    <main v-else class="page">
      ...
      <section v-else-if="mode==='members' && financeSettingsLoadState==='ready' && financeSettingsTab==='surcharges'" class="finance-tax-workspace">
        ...
        <div class="finance-tax-content">
          <section class="tax-country-matrix">
            <header><div><b>国家附加费</b><span>只维护实际报价国家；附加费固定按整张报价单计入一次。</span></div><aside><button class="tax-add-button" type="button" :disabled="!availableSurchargeCountries.length" @click="financeSurchargeCountryAddOpen=true;financeSurchargeCountryAddSearch='';financeSurchargeCountryAdd=availableSurchargeCountries[0]?.country || ''">＋ 添加国家</button><label>⌕<input v-model="financeSurchargeCountrySearch" placeholder="搜索已添加国家"></label></aside></header>
            <div v-if="financeSurchargeCountryAddOpen" class="tax-add-row"><label class="tax-add-search">⌕<input v-model="financeSurchargeCountryAddSearch" autofocus placeholder="输入国家或代码搜索"></label><select v-model="financeSurchargeCountryAdd"><option v-if="!filteredAvailableSurchargeCountries.length" value="" disabled>没有匹配的国家</option><option v-for="setting in filteredAvailableSurchargeCountries" :key="setting.country" :value="setting.country">{{ setting.country }} · {{ financeCountrySettingMap.get(setting.country)?.code || '—' }}</option></select><button class="primary" type="button" :disabled="!financeSurchargeCountryAdd" @click="addSurchargeCountry">确认添加</button><button type="button" @click="financeSurchargeCountryAddOpen=false;financeSurchargeCountryAdd='';financeSurchargeCountryAddSearch=''">取消</button></div>
            ...
            <div class="tax-country-rows">
              ...
              <p v-if="!financeSurchargeCountries.length" class="tax-empty">还没有国家附加费设置，点击“添加国家”开始配置</p>
            </div>
      ...
      <section v-else-if="mode!=='members' || (financeSettingsLoadState==='ready' && financeSettingsTab!=='countries' && financeSettingsTab!=='taxes' && financeSettingsTab!=='surcharges')" class="table-card">
        ...
        <div v-else-if="mode === 'members' && financeSettingsTab==='grades'" class="grade-settings"><header><div><b>客户等级计算系数</b><small>最终报价 = 综合成本 × 客户等级系数；美元报价按财务维护的汇率换算。</small></div><button class="primary" :disabled="savingGradeSettings" @click="saveGradeSettings">{{ savingGradeSettings ? '正在保存…' : '保存等级系数' }}</button></header><div class="grade-grid"><label v-for="setting in customerGradeSettings" :key="setting.grade"><strong>{{ customerGradeDisplayLabel(setting.grade) }}</strong><span>计算系数</span><input v-model.number="setting.coefficient" type="number" min="0.00001" step="0.01"><small>成本 ¥100 → 报价 ¥{{ (100 * setting.coefficient).toFixed(2) }}<template v-if="setting.grade === 'NEW' && !setting.enabled"><br>确认系数后启用，即可用于报价</template></small><em><input v-model="setting.enabled" type="checkbox"> 启用</em></label></div></div>
...
<style scoped>
.tax-eu-scope{padding:12px 18px;border-top:1px solid #e5eaed;font-size:12px;line-height:1.8;color:#64748b}.tax-eu-scope summary{cursor:pointer;color:#a45d00;font-weight:600}.tax-eu-scope p{margin:8px 0 0}.tax-country-matrix>header{flex-wrap:wrap}.tax-country-matrix>header>aside{flex-wrap:wrap}
.finance-channel-cascade>section>header{flex-wrap:wrap}.carrier-bulk-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-left:auto}.carrier-bulk-actions button{border-color:#ffbd63;background:#fff8eb;white-space:nowrap}
...
.finance-stats{grid-template-columns:repeat(6,minmax(0,1fr))}
...
.finance-stats{grid-template-columns:repeat(auto-fit,minmax(min(100%,210px),1fr))}.finance-stats b{white-space:nowrap}.finance-stats span{white-space:normal;line-height:1.35}
</style>
