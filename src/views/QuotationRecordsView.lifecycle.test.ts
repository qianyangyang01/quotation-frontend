...
it('keeps eligible selection during review polling and removes it if another reviewer claims the quote',async()=>{
  await mount()
  const status=document.querySelector<HTMLSelectElement>('.filters select')!
  status.value='finance-pending';status.dispatchEvent(new Event('change'));await flush();await vi.advanceTimersByTimeAsync(250);await flush()
  document.querySelector<HTMLInputElement>('.lifecycle-checkbox')!.click();await flush()
