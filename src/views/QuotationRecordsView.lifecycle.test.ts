...
it('locks won and reviewed rows and clears selection when changing lifecycle',async()=>{
  query.loadRecordPage.mockResolvedValue(result([normal(),{...normal(),id:'won',no:'QT-WON',status:'won'},{...normal(),id:'reviewed',no:'QT-REVIEW',financeReviewStatus:'approved'}]))
  await mount();const checks=document.querySelectorAll<HTMLInputElement>('.lifecycle-checkbox')
  expect(checks[1]!.disabled).toBe(true);expect(checks[2]!.disabled).toBe(true)
...
it('restores to previous archived state and renders archived entries read-only',async()=>{
  ...
  expect(document.querySelector<HTMLSelectElement>('.finance-review')!.disabled).toBe(true)
