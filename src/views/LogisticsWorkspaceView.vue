...
const tab = ref<'prices' | 'imports' | 'history'>('prices')
...
async function refresh() {
  ...
  workspace.value = w; batches.value = b; await loadPrices()
}
...
async function initialize() {
  ...
  datasetId.value ||= datasets.value.find(d => d.status === 'active')?.id || datasets.value[0]?.id || ''
...
async function openBatch(id: string) { await run(async () => { batch.value = await service.batch(id); schedulePoll() }) }
  ...
  <div class="logistics-page">
    ...
    <main>
      ...
      <section v-if="tab === 'prices'" class="card">
        ...
        <div class="scroll"><table><thead><tr><th>物流商 / 渠道</th><th>国家</th><th>重量段</th><th>计费价格</th><th>每票费用</th><th>版本 / 状态</th><th>操作</th></tr></thead><tbody><tr v-for="(r, i) in prices.items" :key="`${r.versionId}-${page}-${i}`"><td><b>{{ r.providerName }}</b><small>{{ r.channelName }}</small></td><td>{{ r.areaName }}<small>{{ r.countryCode }}</small></td><td>{{ weightLabel(r) }}</td><td v-if="r.pricingModel === 'first-next'">首 {{ r.firstWeightKg }}kg / {{ r.currency || 'CNY' }} {{ money(r.firstWeightPrice) }}<small>续 {{ r.nextWeightKg }}kg / {{ r.currency || 'CNY' }} {{ money(r.nextWeightPrice) }}</small></td><td v-else-if="r.intervalPrice">{{ r.currency || 'CNY' }} {{ money(r.intervalPrice) }} / 档</td><td v-else>{{ r.currency || 'CNY' }} {{ money(r.pricePerKg) }} / kg</td><td>{{ r.currency || 'CNY' }} {{ money(r.registrationFee) }}</td><td>V{{ r.versionNumber }}<small :class="{ warning: r.quoteReady === false }">{{ r.quoteReady === false ? '价格已记录 · 计费待适配' : '可自动报价' }}</small></td><td><button :disabled="busy" @click="openVersion(r.versionId!)">查看</button></td></tr><tr v-if="!prices.items.length"><td colspan="7" class="empty">当前条件没有正式价格；新库请先导入并审核价格版本。</td></tr></tbody></table></div>
      ...
      <section v-if="tab === 'imports'" class="stack">
        ...
        <div v-if="batch" class="card"><div class="section-head"><h2>本批处理结果</h2><button :disabled="busy || batch.status !== 'completed'" @click="run(() => service.exportBatchDiff(batch!.id))">导出批次差异</button></div><p>{{ statusLabel[batch.status] }} · {{ statusLabel[batch.phase] || batch.phase }} · {{ batch.payload.progress || 0 }}%<span v-if="batch.payload.elapsedMs"> · 处理 {{ (batch.payload.elapsedMs / 1000).toFixed(2) }} 秒</span></p><progress :value="batch.payload.progress || 0" max="100" /><p v-if="batch.payload.error" class="warning">{{ batch.payload.error }}</p><button v-if="['failed', 'interrupted', 'processing'].includes(batch.status) && !archived" :disabled="busy" @click="run(async () => { batch = await service.retry(batch!.id); schedulePoll() })">重试失败 / 超时批次</button>
          <details v-for="(file, i) in batch.payload.files" :key="i"><summary>{{ file.name }} <button :disabled="busy" @click.stop="run(() => service.original(batch!.id, i, file.name))">下载原文件</button></summary><p v-if="batch.payload.fileReports?.[i]?.message" class="warning">{{ batch.payload.fileReports[i]?.message }}</p><ul><li v-for="s in batch.payload.fileReports?.[i]?.sheets || []" :key="s.name">{{ s.name }}：{{ statusLabel[s.status] || s.status }} · {{ s.priceRows || 0 }} 条价格 {{ s.message || '' }}</li></ul></details>
      ...
      <section v-if="version" class="card version-detail"><div class="section-head"><div><p class="eyebrow">VERSION REVIEW</p><h2>{{ workspace?.channels.find(c => c.id === version?.channelId)?.name }} · V{{ version.versionNumber }}</h2><p>{{ statusLabel[version.status] }} · {{ version.fileName }}</p></div><button @click="version = null">关闭详情</button></div>
        ...
        <div v-if="detailTab === 'diff'" class="scroll"><table><thead><tr><th>国家 / 档位</th><th>类型</th><th>原值 → 新值</th><th>变化</th></tr></thead><tbody><tr v-for="d in visibleDiffs" :key="d.key"><td>{{ d.row.areaName }}<small>{{ weightLabel(d.row) }}</small></td><td>{{ diffLabel[d.type] }}</td><td><div v-for="(c, i) in d.changes" :key="i">{{ c.field }}：{{ shown(c.before) }} → {{ shown(c.after) }}</div><span v-if="!d.changes.length">公斤价 ¥{{ money(d.previous?.pricePerKg) }} → {{ d.type === 'removed' ? '移除' : `¥${money(d.row.pricePerKg)}` }}<small>每票 ¥{{ money(d.previous?.registrationFee) }} → {{ d.type === 'removed' ? '移除' : `¥${money(d.row.registrationFee)}` }}</small></span></td><td><div v-for="(c, i) in d.changes" :key="i">{{ c.delta == null ? '规则变化' : `${c.delta > 0 ? '+' : ''}${money(c.delta)}` }}<small>{{ c.percentChange == null ? '—' : `${c.percentChange > 0 ? '+' : ''}${c.percentChange.toFixed(2)}%` }}</small></div></td></tr></tbody></table></div>
        <div v-if="detailTab === 'rows'" class="scroll"><table><thead><tr><th>国家 / 档位</th><th>公斤价 / 每票费</th><th>首续重 / 区间价</th><th>来源</th><th>待适配原因</th></tr></thead><tbody><tr v-for="(r, i) in visibleRows" :key="i"><td>{{ r.areaName }}<small>{{ weightLabel(r) }}</small></td><td>{{ r.currency || 'CNY' }} {{ money(r.pricePerKg) }} / {{ r.currency || 'CNY' }} {{ money(r.registrationFee) }}</td><td>首 {{ r.currency || 'CNY' }} {{ money(r.firstWeightPrice) }} / 续 {{ r.currency || 'CNY' }} {{ money(r.nextWeightPrice) }}<small>区间 {{ r.currency || 'CNY' }} {{ money(r.intervalPrice) }}</small></td><td>{{ r.sourceSheet }} · 第{{ r.sourceRow }}行</td><td class="warning">{{ r.pendingReason || '—' }}</td></tr></tbody></table></div>
