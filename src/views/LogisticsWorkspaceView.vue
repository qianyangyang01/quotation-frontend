...
import { logisticsRebuild as service, money, shown, weightLabel, completedBatchStage, diffKinds, aggregateChangeSummary, rangeImpact, changeImpact, type Dataset, type Workspace, type Batch, type BatchSummary, type Version, type Cutover, type PricePage, type Diff, type DiffChange, type DiffKind } from '@/data/logisticsRebuild'
  ...
  <div class="logistics-page">
    ...
    <main>
      ...
      <section v-if="tab === 'imports'" class="stack">
        <div v-if="!archived" class="card import-card"><h2>向当前物流库导入价格</h2><p class="import-target"><b>本次导入目标：</b>{{ importTarget }}</p><p><a href="/templates/logistics-v2.xlsx" download="物流标准导入模板V2.xlsx">下载新版标准模板（含示例和填写说明）</a></p><p class="muted">支持 .xls / .xlsx、多工作表、多渠道；每批最多30个文件，总计100MB。文件名无需填写或校验日期，开头日期会自动从显示名称中去除。未审核价格不影响报价。</p><div class="toolbar"><input aria-label="物流报价文件" type="file" accept=".xls,.xlsx" multiple :disabled="busy || !selected" @change="chooseFiles"><button class="primary" :disabled="busy || !selected || !files.length" @click="upload">{{ busy ? '处理中…' : `上传并解析 ${files.length} 份文件` }}</button></div><p v-if="!selected" class="warning">请先选择物流库，再上传价格文件。</p><label class="check"><input v-model="replaceDrafts" type="checkbox">如果已有不同待审稿，确认终止旧稿并保留历史，再生成新稿</label></div>
        ...
        <div v-if="batch" class="card"><div class="section-head"><h2>本批处理结果</h2><button :disabled="busy || batch.status !== 'completed'" @click="run(() => service.exportBatchDiff(batch!.id))">导出批次差异</button></div><p>{{ statusLabel[batch.status] }} · {{ completedBatchStage(batch) || statusLabel[batch.phase] || batch.phase }} · {{ batch.payload.progress || 0 }}%<span v-if="batch.payload.elapsedMs"> · 处理 {{ (batch.payload.elapsedMs / 1000).toFixed(2) }} 秒</span></p><progress :value="batch.payload.progress || 0" max="100" /><p v-if="batch.payload.error" class="warning">{{ batch.payload.error }}</p><button v-if="['failed', 'interrupted', 'processing'].includes(batch.status) && !archived" :disabled="busy" @click="run(async () => { batch = await service.retry(batch!.id); schedulePoll() })">重试失败 / 超时批次</button>
          ...
          <div class="batch-result-summary" aria-label="批次状态汇总"><span>共 <b>{{ batchResultCounts.total }}</b> 个渠道</span><span>待审核 <b>{{ batchResultCounts.draft }}</b></span><span>已发布 <b>{{ batchResultCounts.published }}</b></span><span :class="{ warning: batchResultCounts.blocked > 0 }">阻断 <b>{{ batchResultCounts.blocked }}</b></span></div>
          <div class="change-summary" aria-label="批次变化汇总"><span v-for="key in changeKeys" :key="key" class="change-chip" :class="`change-${key}`">{{ diffLabel[key] }} <b>{{ batchChangeCounts[key] }}</b></span><small>同一价格行可能同时计入多个变化类别。</small></div>
          ...
          <div class="scroll"><table class="batch-results-table"><thead><tr><th>物流商 / 渠道</th><th>结果</th><th>更新摘要</th><th>操作</th></tr></thead><tbody><tr v-for="(r, i) in visibleBatchResults" :key="`${r.versionId || r.channelId || r.channelName}-${i}`"><td>{{ r.providerName }}<small>{{ r.channelName }}</small></td><td>{{ statusLabel[currentBatchResultStatus(r)] || currentBatchResultStatus(r) }}<small class="warning">{{ r.message }}{{ r.quoteReady === false ? '计费规则待适配' : '' }}</small></td><td><div class="inline-change-summary"><span v-for="key in changeKeys" :key="key" :class="`change-${key}`">{{ diffLabel[key] }} {{ changeCount(r.summary, key) }}</span></div><small>{{ r.errors || 0 }} 个阻断问题</small></td><td><button v-if="r.versionId" :disabled="busy" @click="openVersion(r.versionId)">核对版本</button><details v-else><summary>查看原因</summary><p>{{ r.message }}</p><p v-for="(issue, j) in r.issues || []" :key="j">{{ issue.message }}</p></details></td></tr><tr v-if="!visibleBatchResults.length"><td colspan="4" class="empty">当前筛选条件没有渠道结果。</td></tr></tbody></table></div>
