...
import type { LogisticsChannelRecord } from '@/data/logisticsRepository'
import CompanyChannelDirectory from '@/components/logistics/CompanyChannelDirectory.vue'
import type { CompanyDirectory } from '@/data/companyChannels'
import LogisticsPager from '@/components/logistics/LogisticsPager.vue'
import LogisticsIssueSummary from '@/components/logistics/LogisticsIssueSummary.vue'
import type { SourceIssue } from '@/data/logisticsRebuild'
...
import { collectPriceReviewIssues, priceIssueCountries, priceRowsForIssueCountries, priceIssueRowIndexes, prioritizePriceIssueRows, rowsForPriceIssue } from '@/data/logisticsPriceIssueScope'
...
const busy = ref(false), error = ref(''), message = ref(''), note = ref(''), changesConfirmed = ref(false)
const editingRows = ref(false), editSnapshot = ref<Price[]>([])
const showIssueCountries = ref(true)
const locatedPriceIndex = ref(-1)
const issueScopeRows = computed(() => editingRows.value ? editSnapshot.value : version.value?.rows || [])
const reviewIssues = computed(() => collectPriceReviewIssues(issueScopeRows.value, version.value?.issues || [], version.value?.blockingReasons || []))
const reviewErrorCount = computed(() => reviewIssues.value.filter(issue => issue.level === 'error').length)
const rowIssueMessages = computed(() => {
  const result = new Map<Price, string[]>()
  for (const issue of reviewIssues.value.filter(issue => issue.level === 'error')) {
    for (const index of priceIssueRowIndexes(issueScopeRows.value, [issue], false)) {
      const row = version.value?.rows?.[index]
      if (row) result.set(row, [...new Set([...(result.get(row) || []), `${issue.field}：${issue.message}`])])
    }
  }
  return result
})
function locatePriceIssue(issue: SourceIssue) {
  const matched = rowsForPriceIssue(issueScopeRows.value, issue)
  if (!matched.length || busy.value) return
  showIssueCountries.value = false
  detailTab.value = 'rows'
  const actual = version.value?.rows?.[issueScopeRows.value.indexOf(matched[0]!)]
  locatedPriceIndex.value = version.value?.rows?.indexOf(actual!) ?? -1
  detailPage.value = Math.max(0, Math.floor(editorRows.value.indexOf(actual!) / detailPageSize.value))
  void nextTick(() => document.querySelector(`[data-price-index="${locatedPriceIndex.value}"]`)?.scrollIntoView({ block: 'center', behavior: 'smooth' }))
}
watch(() => version.value?.id, () => { locatedPriceIndex.value = -1 })
const issueCountries = computed(() => priceIssueCountries(issueScopeRows.value, version.value?.issues || []))
const editorIssueIndexes = computed(() => priceIssueRowIndexes(issueScopeRows.value, version.value?.issues || []))
const editorIssueRows = computed(() => new Set((version.value?.rows || []).filter((_, index) => editorIssueIndexes.value.has(index))))
const editorRows = computed(() => {
  const rows = version.value?.rows || []
  if (!editingRows.value) return rows
  const sorted = prioritizePriceIssueRows(rows, issueCountries.value, editorIssueIndexes.value)
  return showIssueCountries.value ? priceRowsForIssueCountries(sorted, issueCountries.value) : sorted
})
...
function resetPublishFeedback() { publishFeedback.reset(); publishSummary.value = '' }
watch(() => batch.value?.id, () => { readyPublishConfirmed.value = false })
...
async function channelStatusUpdated(channel: LogisticsChannelRecord) {
  const target = workspace.value?.channels.find(item => item.id === channel.id)
  if (!target || disposed) return
  Object.assign(target, { enabled: channel.enabled, _version: channel._version, updatedAt: channel.updatedAt })
  clearDownload()
  message.value = channel.enabled ? '渠道已启用；是否可报价仍以已发布价格和财务授权为准。' : '渠道已禁用，已停止参与新报价；历史报价、价格版本和财务授权均保留。'
  try { await refresh() } catch { error.value = '渠道状态已保存，但列表刷新失败，请刷新页面核对。' }
}
async function refreshChannelStatus() {
  try { await refresh() } catch { error.value = '无法读取最新渠道状态，请刷新页面后重试。' }
}
...
const visibleRows = computed(() => editorRows.value.slice(detailPage.value * detailPageSize.value, (detailPage.value + 1) * detailPageSize.value))
const detailTotal = computed(() => detailTab.value === 'diff' ? filteredDiffs.value.length : editorRows.value.length)
...
function compactPrice(diff: Diff, source = diff.row) {
  return formattedPrice(source || diff.row)
}
function formattedPrice(price: Price) {
  ...
  if (price.intervalPrice) return `${currency} ${money(price.intervalPrice)} / 票 · 每票费 ${money(price.registrationFee)}${price.pricingModel === 'per-piece-500g' ? ' · 0.5kg进位' : ''}`
  return `${currency} ${money(price.pricePerKg)} / kg · 每票 ${money(price.registrationFee)}${price.pricingModel === 'per-kg-1g' ? ' · 1g进位' : ''}`
}
...
function batchFileState(index: number) {
  ...
  if (report?.status === 'match-pending') return '匹配待确认'
...
async function openCompanyDataset(id: string) {
  if (uploadInFlight.value) { error.value = "请先完成当前上传"; return }
  await run(async () => { selectionEpoch++; datasets.value = await service.datasets(); datasetId.value = id; version.value = null; batch.value = null; page.value = 0; tab.value = "imports"; await refresh() })
}
...
async function changeDetailSize(size: number) { detailPageSize.value = [10, 30, 50].includes(size) ? size : 10; detailPage.value = 0; await nextTick(); resetDetailTableScroll('auto') }
  ...
  await run(async () => {
    ...
    try {
      ...
      if (e instanceof DOMException && e.name === 'AbortError') {
        ...
        message.value = '已暂停上传，已接收分片保留7天；重新选择同一批原文件即可续传。完整文件若已提交，可在待处理记录中查看。'
        return
      }
      throw e
...
function resetBatchResultView() { batchResultQuery.value = ''; batchResultProvider.value = 'all'; batchResultStatus.value = 'all'; batchResultFocus.value = 'all'; batchResultPage.value = 0; readyPublishResult.value = null; readyPublishConfirmed.value = false; resetPublishFeedback() }
function closeBatch() { clearTimeout(pollTimer); forgetActiveBatch(); batch.value = null; version.value = null; resetBatchResultView() }
async function openVersion(id: string) { await run(async () => { resetPublishFeedback(); version.value = await service.version(id); missingEtaMin.value = null; missingEtaMax.value = null; note.value = ''; changesConfirmed.value = false; editingRows.value = false; editSnapshot.value = []; detailPage.value = 0; detailTab.value = version.value.basePublishedVersionId ? 'diff' : 'rows'; diffType.value = 'all'; reviewKey = idempotencyKey('logistics-review'); await nextTick(); versionDetail.value?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }) }
function startEditing() { if (busy.value || !version.value?.rows) return; editSnapshot.value = clonePriceRows(version.value.rows); showIssueCountries.value = issueCountries.value.length > 0; editingRows.value = true; detailTab.value = 'rows'; detailPage.value = 0; resetPublishFeedback(); void nextTick(() => resetDetailTableScroll('auto')) }
async function toggleEditorScope() { showIssueCountries.value = !showIssueCountries.value; detailPage.value = 0; await nextTick(); resetDetailTableScroll('auto') }
function cancelEditing() { if (busy.value) return; if (version.value) version.value.rows = clonePriceRows(editSnapshot.value); editingRows.value = false; editSnapshot.value = [] }
...
function applySuggestion(issue: SourceIssue) { if (!version.value?.rows || !issue.rowKey || !issue.suggestedFields) return; const matches = version.value.rows.filter(item => item.rowKey === issue.rowKey && (!issue.sourceSheet || item.sourceSheet === issue.sourceSheet) && (!issue.row || item.sourceRow === issue.row)); if (matches.length !== 1) { error.value = '问题价格无法唯一定位，请在原表位置对应的记录中手动修正'; return }; if (!editingRows.value) startEditing(); Object.assign(matches[0]!, issue.suggestedFields); detailTab.value = 'rows' }
async function saveCorrections() {
  if (!version.value?.rows) return
  let changes: RowCorrection[], etaChanges
  try { changes = buildPriceCorrections(version.value.rows, editSnapshot.value); etaChanges = buildEtaCorrections(version.value.rows, editSnapshot.value) } catch (e) { error.value = e instanceof Error ? e.message : '修正内容不正确'; return }
  await run(async () => { version.value = await service.patchRows(version.value!, changes, etaChanges); editingRows.value = false; editSnapshot.value = []; changesConfirmed.value = false; await refresh(); if (batch.value && version.value?.batchId === batch.value.id) batch.value = await service.batch(batch.value.id); message.value = reviewErrorCount.value ? `已保存修正并重新校验，仍有 ${reviewErrorCount.value} 个阻断问题。` : version.value!.etaReady === false ? `修正已保存，仍有 ${version.value!.etaMissingCount || 0} 条路线缺少或冲突时效。` : '修正已保存，完整渠道已重新校验并更新差异。' })
}
async function publish() {
  if (busy.value || !version.value || editingRows.value || version.value.errors || version.value.blockingReasons?.length || version.value.pricingReady === false || !note.value.trim() || ((hasCoverageRemoval(version.value.summary) || (version.value.summary.highRisk || 0) > 0) && !changesConfirmed.value)) return
...
onUnmounted(() => { disposed = true; clearTimeout(pollTimer); cancelActiveUpload?.(); publishFeedback.stop() })
</script>
  ...
  <div class="logistics-page">
    <main>
      ...
      <section v-if="tab === 'prices'" class="rule-workspace-card">
        ...
        <form class="modern-filters" @submit.prevent="run(submitPriceFilters)"><label class="keyword-search"><span>⌕</span><input v-model="query" placeholder="物流商＋渠道关键词，如 云途化妆品"></label><label>国家<input v-model="country" placeholder="支持部分名称，如 美 / US"></label><button class="outline-orange" :disabled="busy">查询</button><button type="button" @click="query='';country='';run(submitPriceFilters)">重置</button><span>共 {{ prices.total }} 条正式价格</span></form>
        <div class="scroll" :aria-busy="pricesLoading"><table><thead><tr><th>物流商 / 渠道</th><th>国家</th><th>重量段</th><th>计费价格</th><th>每票费用</th><th>版本 / 状态</th></tr></thead><tbody><template v-if="pricesLoading"><tr v-for="index in 6" :key="`skeleton-${index}`" class="price-skeleton" aria-hidden="true"><td v-for="column in 6" :key="column"><span /></td></tr></template><template v-else><tr v-for="(r, i) in prices.items" :key="`${r.versionId}-${page}-${i}`"><td><b>{{ r.providerName }}</b><small>{{ r.channelName }}</small></td><td>{{ r.areaName }}<small>{{ r.countryCode }} · {{ r.zoneName || '无分区' }}</small></td><td>{{ weightLabel(r) }}<small v-if="r.minChargeWeightKg && r.minChargeWeightKg > 0">最低计费 {{ displayWeightGrams(r.minChargeWeightKg) }}g，不足自动补重</small></td><td v-if="r.pricingModel === 'first-next'">首 {{ r.firstWeightKg }}kg / {{ r.currency || 'CNY' }} {{ money(r.firstWeightPrice) }}<small>续 {{ r.nextWeightKg }}kg / {{ r.currency || 'CNY' }} {{ money(r.nextWeightPrice) }}</small></td><td v-else-if="r.intervalPrice">{{ r.currency || 'CNY' }} {{ money(r.intervalPrice) }} / 档</td><td v-else>{{ r.currency || 'CNY' }} {{ money(r.pricePerKg) }} / kg</td><td>{{ r.currency || 'CNY' }} {{ money(r.registrationFee) }}</td><td>V{{ r.versionNumber }}<small :class="{ warning: r.quoteReady === false }">{{ r.quoteReady === false ? '价格已记录 · 计费待适配' : '可自动报价' }}</small></td></tr><tr v-if="!prices.items.length"><td colspan="6" class="empty">{{ query.trim() || country.trim() ? '没有匹配的运费规则，请减少关键词或调整国家筛选。' : '当前没有正式价格；新库请先导入并审核价格版本。' }}</td></tr></template></tbody></table></div>
      ...
      <section v-if="tab === 'imports' && !version" class="stack">
        ...
        <template v-if="batch">
          ...
          <div class="batch-review-workbench">
            <header class="review-workbench-head">
              ...
              <div class="review-head-actions"><button :disabled="busy || batch.status !== 'completed'" @click="run(() => service.exportBatchStandardized(batch!.id))">导出关键字段</button><button :disabled="busy || batch.status !== 'completed'" @click="run(() => service.exportBatchDiff(batch!.id))">导出差异</button><button :disabled="busy" @click="closeBatch">返回渠道列表</button></div>
            </header>
        ...
        <template v-else>
          <section class="base-settings">
            <div class="base-toolbar"><div><h2>物流商与渠道版本</h2><p>按有效公斤价 × 计费重量＋每票费报价。顺丰优先使用原表折后运费；无折后运费时按折扣计算，两者均为空时按原运费。时效可后补，缺失不影响报价。</p></div><div class="provider-toolbar"><label class="provider-search-field">⌕<input v-model="providerSearch" placeholder="搜索物流商或编码"></label><a class="outline-orange" href="/templates/logistics-v2.xlsx" download="物流标准导入模板V2.xlsx">下载标准模板</a><button class="outline-orange" :disabled="busy || archived" @click="beginUpload('multi')">＋ 多物流商更新导入</button><button @click="showAllHistory">版本历史</button></div></div>
      ...
      <section v-if="version" ref="versionDetail" class="version-review-workbench">
        <header class="review-workbench-head">
          ...
          <button :disabled="busy" @click="version = null">{{ tab === 'history' ? '返回版本历史' : '返回批次审核' }}</button>
        </header>
        ...
        <p v-else-if="version.status === 'draft' && version.pricingReady === false" class="notice error">发现阻断问题或待适配计费条件，请先核对下方原因与原表位置。已解析的价格可在线修正；未识别的原表行需核对原表或解析规则。</p>
        <nav class="review-focus-tabs version-tabs"><button :class="{ active: detailTab === 'diff' }" @click="detailTab = 'diff'; detailPage = 0">只看变化 <b>{{ filteredDiffs.length }}</b></button><button :class="{ active: detailTab === 'rows' }" @click="detailTab = 'rows'; detailPage = 0">{{ editingRows && showIssueCountries && issueCountries.length ? '问题国家价格' : '完整价格' }} <b>{{ editingRows ? editorRows.length : version.rows?.length || 0 }}</b></button><button class="issue-tab" :class="{ active: detailTab === 'issues' }" @click="detailTab = 'issues'; detailPage = 0">价格 / 重量问题 <b>{{ reviewIssues.length }}</b></button></nav>
        <div class="version-review-layout">
          <section class="review-main-panel">
            <LogisticsIssueSummary :rows="issueScopeRows" :issues="reviewIssues" :busy="busy" :can-suggest="version.status === 'draft'" @locate="locatePriceIssue" @suggest="applySuggestion" />
            <div class="version-toolbar"><div><button :disabled="busy" @click="run(() => service.exportPrices(datasetId, versionFilters(version!.id)))">导出价格</button><button :disabled="busy" @click="run(() => service.exportVersionStandardized(version!))">导出关键字段</button><button :disabled="busy" @click="run(() => service.exportDiff(version!))">导出差异</button><button v-if="version.batchId" :disabled="busy" @click="run(() => service.original(version!.batchId!, version!.sourceFileIndex || 0))">查看原表</button></div><div><select v-if="detailTab === 'diff'" v-model="diffType" aria-label="差异类型" @change="detailPage = 0"><option value="all">全部差异类型</option><option v-for="(label, key) in diffLabel" :key="key" :value="key">{{ label }}</option></select><button v-if="version.status === 'draft' && !editingRows" class="primary" :disabled="busy" @click="startEditing">修正价格 / 重量区间</button><template v-if="editingRows"><button :disabled="busy" @click="cancelEditing">取消修正</button><button class="primary" :disabled="busy" @click="saveCorrections">保存并重新校验</button></template></div></div>
            <p class="muted">对比基线：{{ version.basePublishedVersionId || '初始价格版本（不与旧库比较）' }}</p>
            <LogisticsPager v-if="detailTab !== 'issues' && detailTotal" class="detail-pager detail-pager-top" :page="detailPage" :size="detailPageSize" :total="detailTotal" :total-pages="detailTotalPages" :loading="busy" :size-options="[10, 30, 50]" aria-label="价格明细顶部分页" @page-change="changeDetailPage" @size-change="changeDetailSize" />
            <div v-if="editingRows" class="editor-scope" role="status"><div><b v-if="showIssueCountries && issueCountries.length">已定位问题国家：{{ issueCountries.map(country => country.label).join('、') }}</b><b v-else>正在查看全部国家</b><p>{{ showIssueCountries && issueCountries.length ? `问题档位已置顶并标红，共 ${editorIssueRows.size} 条；下方保留该国家其余重量段供对照。` : '可以修改价格和重量区间，切换范围不会丢失已修改内容。' }}</p><p v-if="reviewErrorCount && !issueCountries.length">当前问题未匹配到已解析价格行，请查看上方列出的原表位置和原因。</p></div><button v-if="issueCountries.length" :disabled="busy" @click="toggleEditorScope">{{ showIssueCountries ? `查看全部 ${version.rows?.length || 0} 条价格` : '返回问题国家' }}</button></div>
            ...
            <div v-if="detailTab === 'rows'" class="scroll"><table v-if="!editingRows"><thead><tr><th>国家 / 档位</th><th>计费价格 / 每票费</th><th>来源</th><th>校验结果</th></tr></thead><tbody><tr v-for="(r, i) in visibleRows" :key="i" :data-price-index="version.rows?.indexOf(r)" :class="{ 'row-has-issue': editorIssueRows.has(r), 'row-location-target': locatedPriceIndex === version.rows?.indexOf(r) }"><td><b>{{ r.areaName }} · {{ r.zoneName || '无分区' }}</b><small>{{ weightLabel(r) }}</small></td><td>{{ formattedPrice(r) }}<small v-if="r.minChargeWeightKg">最低计费 {{ displayWeightGrams(r.minChargeWeightKg) }}g</small></td><td>{{ r.sourceSheet }} · 第{{ r.sourceRow }}行<small>{{ r.etaStatus && r.etaStatus !== 'ready' || !r.etaMinDays || !r.etaMaxDays ? '该物流暂无时效说明' : `时效 ${r.etaMinDays}–${r.etaMaxDays} 天` }}</small></td><td :class="{ warning: r.pendingReason || rowIssueMessages.has(r) }"><p v-for="problem in rowIssueMessages.get(r) || []" :key="problem" class="issue-reason">{{ problem }}</p>{{ r.pendingReason || (rowIssueMessages.has(r) ? '' : '✓ 价格校验通过') }}<details v-if="r.reviewWarning"><summary>原表说明</summary><small>{{ r.reviewWarning }}</small></details></td></tr></tbody></table><table v-else class="price-editor"><thead><tr><th>国家 / 分区</th><th>起重kg</th><th>含起点</th><th>止重kg</th><th>含终点</th><th>计费价格</th><th>每票费</th><th>时效 / 原表位置</th></tr></thead><tbody><tr v-for="r in visibleRows" :key="version.rows?.indexOf(r)" :data-price-index="version.rows?.indexOf(r)" :class="{ 'row-location-target': locatedPriceIndex === version.rows?.indexOf(r), 'row-has-issue': editorIssueRows.has(r), 'row-issue-context': showIssueCountries && !editorIssueRows.has(r) }"><td><b>{{ r.areaName }}</b><strong v-if="editorIssueRows.has(r)" class="editor-issue-tag">问题档位 · 优先核对</strong><small v-for="problem in rowIssueMessages.get(r) || []" :key="problem" class="issue-reason">{{ problem }}</small><small v-if="!editorIssueRows.has(r) && showIssueCountries && issueCountries.length" class="editor-context-tag">同国参考档位</small><small>{{ r.zoneName || '无分区' }}</small></td><td><input v-model.number="r.weightFromKg" type="number" min="0" step="0.001"></td><td><input v-model="r.weightFromInclusive" type="checkbox"></td><td><input v-model.number="r.weightToKg" type="number" min="0" step="0.001"></td><td><input v-model="r.weightToInclusive" type="checkbox"></td><td><input v-if="r.pricingModel === 'per-piece-500g'" v-model.number="r.intervalPrice" aria-label="整票档位价" type="number" min="0" step="0.01"><input v-else v-model.number="r.pricePerKg" aria-label="公斤价" type="number" min="0" step="0.01"><small>{{ r.pricingModel === 'per-piece-500g' ? '元/票，0.5kg进位' : '元/kg' }}</small></td><td><input v-model.number="r.registrationFee" type="number" min="0" step="0.01"></td><td><input v-model.number="r.etaMinDays" aria-label="时效最早天数" type="number" min="1" max="365" step="1" @change="syncEtaRoute(r)"><input v-model.number="r.etaMaxDays" aria-label="时效最晚天数" type="number" min="1" max="365" step="1" @change="syncEtaRoute(r)"><small>{{ r.sourceSheet }} · 第{{ r.sourceRow }}行</small></td></tr></tbody></table></div>
            <p v-if="detailTab === 'issues' && !reviewIssues.length" class="issue-empty">✓ 没有解析或区间问题</p>
            <LogisticsPager v-if="detailTab !== 'issues' && detailTotal" class="detail-pager detail-pager-bottom" :page="detailPage" :size="detailPageSize" :total="detailTotal" :total-pages="detailTotalPages" :loading="busy" :size-options="[10, 30, 50]" aria-label="价格明细底部分页" @page-change="changeDetailPage" @size-change="changeDetailSize" />
          </section>
...
.issue-reason{display:block;color:#b32e25;font-weight:600;white-space:normal;line-height:1.5}.price-editor .row-has-issue input{border-color:#df655c;background:#fff8f7}
.row-location-target{outline:2px solid #3977cb;outline-offset:-2px}
</style>
