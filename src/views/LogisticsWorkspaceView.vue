...
import { priceIssueCountries, priceRowsForIssueCountries, priceIssueRowIndexes, prioritizePriceIssueRows, rowsForPriceIssue } from '@/data/logisticsPriceIssueScope'
...
const reviewIssues = computed(() => {
  const issues = [...(version.value?.issues || [])]
  for (const row of issueScopeRows.value) if (row.blockingReason && !issues.some(issue => rowsForPriceIssue(issueScopeRows.value, issue).includes(row) && issue.message.includes(row.blockingReason!))) {
    issues.push({ row: row.sourceRow || 0, sourceSheet: row.sourceSheet, rowKey: row.rowKey, field: '计费规则', message: row.blockingReason, level: 'error' })
  }
  for (const reason of version.value?.blockingReasons || []) if (!issues.some(issue => issue.message.includes(reason))) issues.push({ row: 0, field: '渠道规则', message: reason, level: 'error' })
  return issues
})
  ...
  <div class="logistics-page">
    <main>
      ...
      <section v-if="version" ref="versionDetail" class="version-review-workbench">
        ...
        <nav class="review-focus-tabs version-tabs"><button :class="{ active: detailTab === 'diff' }" @click="detailTab = 'diff'; detailPage = 0">只看变化 <b>{{ filteredDiffs.length }}</b></button><button :class="{ active: detailTab === 'rows' }" @click="detailTab = 'rows'; detailPage = 0">{{ editingRows && showIssueCountries && issueCountries.length ? '问题国家价格' : '完整价格' }} <b>{{ editingRows ? editorRows.length : version.rows?.length || 0 }}</b></button><button class="issue-tab" :class="{ active: detailTab === 'issues' }" @click="detailTab = 'issues'; detailPage = 0">价格 / 重量问题 <b>{{ version.issues?.length || 0 }}</b></button></nav>
        <div class="version-review-layout">
          <section class="review-main-panel">
            <LogisticsIssueSummary :rows="issueScopeRows" :issues="detailTab === 'issues' ? reviewIssues : reviewIssues.filter(issue => issue.level === 'error')" :busy="busy" :can-suggest="version.status === 'draft'" @locate="locatePriceIssue" @suggest="applySuggestion" />
            ...
            <div v-if="editingRows" class="editor-scope" role="status"><div><b v-if="showIssueCountries && issueCountries.length">已定位问题国家：{{ issueCountries.map(country => country.label).join('、') }}</b><b v-else>正在查看全部国家</b><p>{{ showIssueCountries && issueCountries.length ? `问题档位已置顶并标红，共 ${editorIssueRows.size} 条；下方保留该国家其余重量段供对照。` : '可以修改价格和重量区间，切换范围不会丢失已修改内容。' }}</p><p v-if="version.errors && !issueCountries.length">当前问题未匹配到已解析价格行，请查看上方列出的原表位置和原因。</p></div><button v-if="issueCountries.length" :disabled="busy" @click="toggleEditorScope">{{ showIssueCountries ? `查看全部 ${version.rows?.length || 0} 条价格` : '返回问题国家' }}</button></div>
