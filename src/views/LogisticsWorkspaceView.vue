  ...
  <div class="logistics-page">
    <main>
      ...
      <section v-if="version" ref="versionDetail" class="version-review-workbench">
        ...
        <p v-else-if="version.status === 'draft' && version.etaReady === false" class="notice error">有 {{ version.etaMissingCount || version.missingEtaRoutes?.length || 0 }} 条路线缺少或存在冲突时效。请在现有价格明细编辑中填写一次，系统会同步整条路线。</p>
        ...
        <div class="version-review-layout">
          <section class="review-main-panel">
            ...
            <div v-if="detailTab === 'rows'" class="scroll"><table v-if="!editingRows"><thead><tr><th>国家 / 档位</th><th>公斤价 / 每票费</th><th>首续重 / 区间价</th><th>来源</th><th>校验结果</th></tr></thead><tbody><tr v-for="(r, i) in visibleRows" :key="i"><td><b>{{ r.areaName }} · {{ r.zoneName || '无分区' }}</b><small>{{ weightLabel(r) }}</small></td><td>{{ r.currency || 'CNY' }} {{ money(r.pricePerKg) }} / {{ r.currency || 'CNY' }} {{ money(r.registrationFee) }}</td><td>首 {{ r.currency || 'CNY' }} {{ money(r.firstWeightPrice) }} / 续 {{ r.currency || 'CNY' }} {{ money(r.nextWeightPrice) }}<small>区间 {{ r.currency || 'CNY' }} {{ money(r.intervalPrice) }}</small></td><td>{{ r.sourceSheet }} · 第{{ r.sourceRow }}行<small>时效 {{ r.etaMinDays || '—' }}–{{ r.etaMaxDays || '—' }} 天</small></td><td :class="{ warning: r.pendingReason || r.reviewWarning }">{{ r.pendingReason || r.reviewWarning || '✓ 通过' }}</td></tr></tbody></table><table v-else class="price-editor"><thead><tr><th>国家 / 分区</th><th>起重kg</th><th>含起点</th><th>止重kg</th><th>含终点</th><th>公斤价</th><th>每票费</th><th>首重kg / 价</th><th>续重kg / 价</th><th>区间价</th><th>时效 / 原表位置</th></tr></thead><tbody><tr v-for="r in visibleRows" :key="r.rowKey" :class="{ 'row-has-issue': r.pendingReason }"><td><b>{{ r.areaName }}</b><small>{{ r.zoneName || '无分区' }}</small></td><td><input v-model.number="r.weightFromKg" type="number" min="0" step="0.001"></td><td><input v-model="r.weightFromInclusive" type="checkbox"></td><td><input v-model.number="r.weightToKg" type="number" min="0" step="0.001"></td><td><input v-model="r.weightToInclusive" type="checkbox"></td><td><input v-model.number="r.pricePerKg" type="number" min="0" step="0.01"></td><td><input v-model.number="r.registrationFee" type="number" min="0" step="0.01"></td><td><input v-model.number="r.firstWeightKg" type="number" min="0" step="0.001"><input v-model.number="r.firstWeightPrice" type="number" min="0" step="0.01"></td><td><input v-model.number="r.nextWeightKg" type="number" min="0" step="0.001"><input v-model.number="r.nextWeightPrice" type="number" min="0" step="0.01"></td><td><input v-model.number="r.intervalPrice" type="number" min="0" step="0.01"></td><td><input v-model.number="r.etaMinDays" aria-label="时效最早天数" type="number" min="1" max="365" step="1" @change="syncEtaRoute(r)"><input v-model.number="r.etaMaxDays" aria-label="时效最晚天数" type="number" min="1" max="365" step="1" @change="syncEtaRoute(r)"><small>{{ r.sourceSheet }} · 第{{ r.sourceRow }}行</small></td></tr></tbody></table></div>
