...
function resetBatchResultView() { batchResultQuery.value = ''; batchResultProvider.value = 'all'; batchResultStatus.value = 'all'; batchResultFocus.value = 'all'; batchResultPage.value = 0; readyPublishResult.value = null; readyPublishConfirmed.value = false }
function closeBatch() { clearTimeout(pollTimer); forgetActiveBatch(); batch.value = null; version.value = null; resetBatchResultView() }
async function openVersion(id: string) { await run(async () => { version.value = await service.version(id); missingEtaMin.value = null; missingEtaMax.value = null; note.value = ''; changesConfirmed.value = false; editingRows.value = false; editSnapshot.value = []; detailPage.value = 0; detailTab.value = version.value.basePublishedVersionId ? 'diff' : 'rows'; diffType.value = 'all'; reviewKey = idempotencyKey('logistics-review'); await nextTick(); versionDetail.value?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }) }
...
async function publish() {
  if (!version.value || ((hasCoverageRemoval(version.value.summary) || (version.value.summary.highRisk || 0) > 0) && !changesConfirmed.value)) return
  await run(async () => { version.value = await service.review(version.value!, note.value, changesConfirmed.value, changesConfirmed.value, reviewKey); reviewKey = idempotencyKey('logistics-review'); await invalidatePublishedLogisticsCache(); await refresh(); message.value = version.value.quoteReady === false ? '价格已保存为正式版本；渠道仍待适配，不开放自动报价。' : '新价格已生效。' })
}
...
async function publishReadyBatch() {
  if (!batch.value || !readyBatchResults.value.length || !readyPublishNote.value.trim() || ((readyBatchNeedsRemoval.value || readyBatchNeedsRisk.value) && !readyPublishConfirmed.value)) return
