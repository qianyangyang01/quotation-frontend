...
<template>
  <div class="logistics-page">
    <AppTopbar />
    <main>
      <header class="milano-heading"><div><p>LOGISTICS CONFIGURATION</p><h1>物流规则</h1><span>维护物流渠道、国家区域、重量限制与分段运费，供米莱诺报价计算直接调用。</span></div><div class="dataset-picker"><label>正在查看的物流库<select v-model="datasetId" :disabled="busy" @change="changeDataset"><option v-for="d in datasets" :key="d.id" :value="d.id">{{ d.name }} · {{ statusLabel[d.status] }}</option></select></label><span class="tag" :class="selected?.status">{{ statusLabel[selected?.status || ''] }}</span></div></header>
