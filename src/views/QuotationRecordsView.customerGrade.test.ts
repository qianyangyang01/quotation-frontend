...
it.each(['mine', 'company'] as const)('shows saved customer grades below the quotation number in %s records', async scope => {
  ...
  for (const el of host.querySelectorAll('.record-customer-grade')) expect(el.previousElementSibling?.classList.contains('record-number')).toBe(true)
})
