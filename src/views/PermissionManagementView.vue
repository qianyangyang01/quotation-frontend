...
const firstRow = computed(() => filteredUsers.value.length ? (page.value - 1) * pageSize.value + 1 : 0)
const lastRow = computed(() => Math.min(page.value * pageSize.value, filteredUsers.value.length))
watch([keyword, roleFilter, pageSize], () => { page.value = 1 })
const notice = ref('')
...
<style scoped>
.accounts-pagination{display:flex;align-items:center;justify-content:space-between;gap:12px;padding-top:16px;color:#687782;font-size:12px}.accounts-pagination>div,.accounts-pagination label{display:flex;align-items:center;gap:8px}.accounts-pagination button{height:34px;padding:0 12px;border:1px solid #d6dee4;border-radius:6px;background:#fff;color:#233441;cursor:pointer}.accounts-pagination button:disabled{color:#aeb7bf;cursor:default}.accounts-pagination button:focus-visible{outline:2px solid #df8508;outline-offset:2px}@media(max-width:720px){.accounts-pagination,.accounts-pagination>div{flex-wrap:wrap}}
