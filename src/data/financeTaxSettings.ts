...
export function calculateFinanceQuoteTax(
  ...
  const noTax = !countrySetting?.enabled || finiteNonNegative(countrySetting.fixedFeeUsd) === 0
  const included = !noTax && mode === 'exempt'
  const configured = noTax || Boolean(mode)
  ...
  return {
    ...
    ratePercent: null,
    fixedFeeUsd,
    feeMode: noTax ? 'no-tax' : !configured ? 'missing' : included ? 'exempt' : 'fixed-order',
    taxUsd,
    totalUsd,
    label: noTax ? '无关税' : !configured ? (!mode ? '渠道关税属性待设置' : `${country || '当前国家'}关税待设置`) : included ? '免税' : `关税 $${fixedFeeUsd.toFixed(2)}/单`,
