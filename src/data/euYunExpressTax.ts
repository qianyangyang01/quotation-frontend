...
export type EuYunExpressTaxContext = { channelKey: string; weightKg?: number; eurUsd?: number }
...
export function calculateEuYunExpressTax(country: string, provider: string, baseUsd: number, context?: EuYunExpressTaxContext): FinanceQuoteTaxResult | null {
  ...
  return {
    ...
    label: `关税 $${taxUsd.toFixed(2)}/单（计费重 ${decimal(weightKg!).times(1000).toNumber()}g）`,
