...
export function quotationRecordQuoteOnlyLayout(record: QuotationRecord): { text: string; html: string } {
  ...
  ])].filter(q => Number.isSafeInteger(q) && q >= 0).sort((a, b) => (a || Infinity) - (b || Infinity))
  ...
  for (const option of options) {
    const country = option.quoteRegion && option.quoteRegion !== '全国统一' && option.quoteRegion !== option.country ? `${option.country} · ${option.quoteRegion}` : option.country
    const customer = snapshot?.rows.find(row => row.optionId === option.id)
    const prices = quantities.map(quantity => {
      const index = snapshot?.quantities.indexOf(quantity) ?? -1
      const value = snapshot ? index >= 0 ? customer?.prices[index] : null : savedSystemPrice(record, option, quantity)
  ...
  const htmlRows = rows.map(row => `<tr>${row.cells.map((value, i) => {
    const span = row.kind === 'metadata' && i === 5 ? columnCount - 5 : row.kind === 'note' ? columnCount : 1
