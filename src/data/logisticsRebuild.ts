...
import type { ImportCoverage } from './logisticsImportCoverage'
...
export type ReadyPublishResult = { coverage?: ImportCoverage; batchId: string; publishedCount: number; skippedCount: number; failedCount: number; published: Array<{ versionId: string; channelId: string; providerName: string; channelName: string; message: string }>; skipped: Array<{ versionId: string; channelName: string; reason: string }>; failed: Array<{ versionId: string; channelName: string; reason: string }> }
...
export const logisticsRebuild = {
  ...
  publishReady: (batchId: string, selections: BatchPublishSelection[], note: string, key: string, coverage: { coverageToken: string; partialUpdateConfirmed: boolean }) => api.post<ReadyPublishResult>(`${root}/imports/${batchId}/publish-ready`, { selections, note, ...coverage }, key),
  ...
  exportAllChannels: (id: string) => downloadFile(new URLSearchParams({ kind: 'all-channel-prices', id })),
