...
describe('country surcharge and channel exemptions', () => {
  ...
  it('preserves explicitly configured zero and disabled duty without changing legacy missing duty', () => {
    const config = fees()
    ...
    expect(calculateFinanceQuoteTax(normalizeFinanceTaxSettings(config), '美国', '应税物流', 20).configured).toBe(false)
  })
