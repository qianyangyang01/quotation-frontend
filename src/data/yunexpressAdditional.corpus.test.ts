...
const path = process.env.YUNEXPRESS_BILLING_CASES
it.skipIf(!path)('matches the four Yunexpress source tariffs and gram rounding boundaries', () => {
  ...
  const rules = new Map(evidence.channels.map((c, i) => [c.channelName, {
    ...
    relations: [{ carrier: c.providerName, channel: c.channelName, channelCode: `YT${i}`, discounts: '' }],
  ...
  expect(evidence.cases.length).toBeGreaterThan(650)
