...
export interface PurchaseImportRowView { sourceRow:number;sku:string;status:string;action:string;error?:string;payload:Record<string,unknown> }
...
export const confirmPurchaseImportJob=(id:string)=>api.post<{jobId:string;status:string}>(`/purchase-imports/jobs/${id}/confirm`,undefined,idempotencyKey('purchase-async-confirm'))
