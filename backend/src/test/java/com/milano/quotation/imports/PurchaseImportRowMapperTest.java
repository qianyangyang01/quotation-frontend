...
class PurchaseImportRowMapperTest {
    ...
    @Test void reportsRequiredInvalidAndOptionalWarnings(){
        ...
        assertTrue(row.errors().size()>=8);assertTrue(row.warnings().size()>=3);assertEquals("待补充资料",row.payload().path("status").asText());assertTrue(row.payload().path("weightKg").isNull());
    }
