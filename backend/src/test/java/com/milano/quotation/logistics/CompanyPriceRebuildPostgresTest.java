...
class CompanyPriceRebuildPostgresTest {
    ...
    @org.junit.jupiter.api.io.TempDir java.nio.file.Path objectFolder;
    ...
    @Test void pausesBacksUpAndPhysicallyDeletesOnlyOldPricesWithoutChangingHistoricalQuote() throws Exception {
        var objects=new HashMap<String,java.nio.file.Path>();var storage=mock(AssetStorageService.class);
        doAnswer(call->{var file=java.nio.file.Files.createTempFile(objectFolder,"object-",".ndjson");try(var out=java.nio.file.Files.newOutputStream(file)){((InputStream)call.getArgument(1)).transferTo(out);}objects.put(call.getArgument(0),file);return null;}).when(storage).putRaw(anyString(),any(),anyLong(),anyString());
        when(storage.openRaw(anyString())).thenAnswer(call->new FilterInputStream(java.nio.file.Files.newInputStream(Objects.requireNonNull(objects.get(call.getArgument(0))))) {
            @Override public byte[] readAllBytes(){throw new AssertionError("Full backup buffering is forbidden");}
        });
        when(storage.removeRaw(anyString())).thenAnswer(call->{var file=objects.remove(call.getArgument(0));return file==null||java.nio.file.Files.deleteIfExists(file);});
        int versions=Boolean.getBoolean("company.largeBackup")?961:1;
        tx.executeWithoutResult(status->{try{
            ...
            if(versions>1)jdbc.sql("insert into logistics_version(id,channel_id,version_number,status,source_hash,payload,created_at) select gen_random_uuid(),:c,n,'draft','large-'||n,jsonb_build_object('rows','[]'::jsonb,'padding',repeat('x',800000)),now() from generate_series(2,:last) n").param("c",channel).param("last",versions).update();
            ...
            var preview=rebuild.preview();assertEquals(versions,preview.path("priceVersions").asInt());
            ...
            assertEquals("ndjson-v1",saved.path("payload").path("backup").path("format").asText());
            try(var lines=java.nio.file.Files.lines(objects.get(backupKey))){assertEquals(versions,lines.filter(line->mapper.readTree(line).path("table").asText().equals("logistics_version")).count());}
            if(versions>1){assertTrue(java.nio.file.Files.size(objects.get(backupKey))>740_000_000);System.out.println("Large streamed backup bytes="+java.nio.file.Files.size(objects.get(backupKey))+", maxHeap="+Runtime.getRuntime().maxMemory());}
            ...
            assertEquals(versions,jdbc.sql("select count(*) from logistics_version").query(Integer.class).single());
            assertEquals(before,jdbc.sql("select payload::text from quotation_record where id=:id").param("id",quote).query(String.class).single());
        }catch(IOException e){throw new UncheckedIOException(e);}finally{status.setRollbackOnly();}});
    }
}
