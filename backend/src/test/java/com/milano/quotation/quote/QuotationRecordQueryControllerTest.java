...
class QuotationRecordQueryControllerTest {
    ...
    @Test void passesIndependentFiltersAndUsesAuthenticatedReviewer() {
        ...
        controller.search("company",1,30,"SKU","processed","reviewing",true,"美国","服装","SKU-1","Alice","专线","multiple","lower","active",null,null,auth("allRecords"));
        verify(query).search(isNull(),argThat(f->f.status().equals("processed")&&f.reviewStatus().equals("reviewing")&&f.reviewMine()&&f.reviewer().equals("ME")&&f.q().equals("SKU")&&f.country().equals("美国")&&f.product().equals("SKU-1")&&f.customer().equals("Alice")&&f.channel().equals("专线")&&f.optionScale().equals("multiple")&&f.priceDifference().equals("lower")),eq(1),eq(30));
    }
}
