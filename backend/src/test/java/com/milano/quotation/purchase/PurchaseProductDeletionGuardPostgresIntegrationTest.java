...
class PurchaseProductDeletionGuardPostgresIntegrationTest {
    ...
    @Test void countsOnlyStructuredBusinessReferences() {
        ...
        var product=UUID.randomUUID();var job=UUID.randomUUID();
        ...
        assertFalse(check.canDelete());assertEquals(7,check.version());
        ...
        jdbc.update("delete from quotation_record");jdbc.update("delete from quotation_draft");jdbc.update("delete from quotation_template");jdbc.update("update purchase_import_row set rolled_back_at=now()");
