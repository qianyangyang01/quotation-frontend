...
class PurchaseProductServiceTest {
    ...
    @Test void pastedRowsValidateBeforeWritingAndNeverOverwriteExistingSku() {
        ...
        assertEquals(1,service.createPasted(List.of(pasted("P-3"),overwrite)).size());
        assertTrue(service.createPasted(List.of(overwrite)).isEmpty());
        assertTrue(rows.containsKey("P-3")); assertEquals(9.24,rows.get("P-1").payload.path("purchasePriceCny").asDouble());
    }
    ...
    @Test void pastedDuplicatesKeepFirst() {
        var result=service.createPasted(List.of(pasted("P-1"),pasted("p-1").put("purchasePriceCny",100)));
        assertEquals(1,result.size());
        assertEquals(9.24,result.getFirst().path("purchasePriceCny").asDouble());
    }
