...
BEGIN
    ...
    IF state.paused AND selected_dataset IS DISTINCT FROM state.target_dataset_id THEN
        ...
        IF NOT (job.phase='deleting' AND current_setting('app.logistics_purge_job',true)=job.id::text
          AND job.payload->'sourceDatasets' @> to_jsonb(ARRAY[selected_dataset::text])) THEN
          RAISE EXCEPTION '物流重建中，旧价格暂停写入';
        END IF;
    END IF;
    IF TG_TABLE_NAME='logistics_version' AND TG_OP<>'DELETE' AND NEW.status IN ('draft','published')
       AND NOT logistics_company_allowed(selected_channel) THEN
...
BEGIN
    ...
    IF dataset_status='archived' THEN
      SELECT EXISTS(SELECT 1 FROM logistics_company_rebuild j JOIN logistics_company_state s ON s.rebuild_id=j.id
        WHERE s.paused AND j.phase='deleting' AND j.id::text=current_setting('app.logistics_purge_job',true)
        AND j.payload->'sourceDatasets' @> to_jsonb(ARRAY[selected_dataset::text])) INTO purge;
      IF NOT purge OR (TG_OP<>'DELETE' AND NOT(TG_TABLE_NAME='logistics_channel' AND TG_OP='UPDATE' AND NEW.current_version_id IS NULL))
        THEN RAISE EXCEPTION 'Archived logistics dataset is read-only'; END IF;
    END IF;
    IF TG_OP='DELETE' THEN RETURN OLD; END IF; RETURN NEW;
END $$;
