-- Explicit gram rounding for newly checked kilogram tariffs; preserve old approvals and data.
ALTER FUNCTION logistics_price_row_quote_supported(JSONB) RENAME TO logistics_price_row_quote_supported_v43;
CREATE FUNCTION logistics_price_row_quote_supported(price JSONB) RETURNS BOOLEAN LANGUAGE sql IMMUTABLE AS $$
SELECT CASE WHEN price->>'pricingModel'='per-kg-1g' THEN
  logistics_price_row_quote_supported_v43(price || '{"pricingModel":"per-kg"}'::jsonb)
  AND coalesce(price->>'currency','CNY')='CNY'
  ELSE logistics_price_row_quote_supported_v43(price) END;
$$;
...
SELECT EXISTS (
  ...
  WHERE v.id=selected_version AND v.status='published' AND a.rows_fingerprint=v.rows_fingerprint
    AND ((a.kind IN ('verified','validated-import') AND a.engine_version IN ('logistics-billing-v3','logistics-billing-v4','logistics-billing-v5','logistics-billing-v6','logistics-billing-v7'))
    ...
    AND (NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v.quote_rows) item WHERE item->>'pricingModel'='per-piece-500g')
         OR a.engine_version IN ('logistics-billing-v6','logistics-billing-v7'))
    AND (NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v.quote_rows) item WHERE item->>'pricingModel'='per-kg-1g')
         OR a.engine_version='logistics-billing-v7')
);
$$;
