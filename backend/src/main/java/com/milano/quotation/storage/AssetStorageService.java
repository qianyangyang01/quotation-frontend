...
public class AssetStorageService{
    ...
    public void putRaw(String objectKey,InputStream stream,long size,String contentType){putRawWithSha256(objectKey,stream,size,contentType);}
    public String putRawWithSha256(String objectKey,InputStream stream,long size,String contentType){try{var digest=MessageDigest.getInstance("SHA-256");var digesting=new java.security.DigestInputStream(stream,digest);minio.putObject(PutObjectArgs.builder().bucket(bucket).object(objectKey).stream(digesting,size,-1).contentType(contentType).build());return HexFormat.of().formatHex(digest.digest());}catch(Exception e){throw new AppException(HttpStatus.SERVICE_UNAVAILABLE,"STORAGE_UNAVAILABLE","迁移文件存储失败");}}
