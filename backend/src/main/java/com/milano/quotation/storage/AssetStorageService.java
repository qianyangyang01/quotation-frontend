...
public class AssetStorageService{
    ...
    @Transactional public AssetObject storeImage(byte[] bytes,String originalName){if(bytes.length==0||bytes.length>MAX_IMAGE_BYTES)throw AppException.unprocessable("图片为空或超过20MB");var mediaType=detectImage(bytes);var sha=sha256(bytes);var existing=assets.findBySha256(sha);if(existing.isPresent())return existing.get();var objectKey="objects/"+sha.substring(0,2)+"/"+sha;try{minio.putObject(PutObjectArgs.builder().bucket(bucket).object(objectKey).stream(new ByteArrayInputStream(bytes),bytes.length,-1).contentType(mediaType).build());}catch(Exception e){throw new AppException(HttpStatus.SERVICE_UNAVAILABLE,"STORAGE_UNAVAILABLE","图片存储暂时不可用");}var row=new AssetObject();row.id=UUID.randomUUID();row.sha256=sha;row.objectKey=objectKey;row.mediaType=mediaType;row.sizeBytes=bytes.length;row.originalName=safeName(originalName);row.createdAt=Instant.now();return assets.save(row);}
