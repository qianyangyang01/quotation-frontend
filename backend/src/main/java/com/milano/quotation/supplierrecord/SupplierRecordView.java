...
public record SupplierRecordView(
    ...
    static SupplierRecordView from(SupplierRecord row) {
        return new SupplierRecordView(row.id, row.name, empty(row.industryBelt), empty(row.contactRole),
                empty(row.relationshipNotes), empty(row.invoiceType), row.taxPoint, empty(row.qualityGrade),
                empty(row.deliveryTerms), empty(row.capacityOrder), empty(row.stockingStrategy),
                empty(row.alternativeInquiry), empty(row.costSheet), row.hotProductRecommendation, row.freeSample,
