...
public class QuotationLifecycleController {
    ...
    public QuotationLifecycleController(QuotationRecordRepository records, AuditService audit) {
    ...
    public ApiResponse<Result> change(@RequestBody Request request, Authentication auth) {
        ...
        for (var entry : versions.entrySet()) {
            ...
            if (request.action().equals("restore")) {
                ...
                if (protectedRecord(row)) throw AppException.conflict("包含已成交、已有成交明细或已审核记录，本次未执行；请管理员单独核实");
            }
            rows.add(row);
        }
        var now = Instant.now();
    ...
    static boolean protectedRecord(QuotationRecordEntity row) {
        JsonNode payload = row.payload;
        var review = payload.path("financeReviewStatus").asText("pending");
