...
public class QuotationCountryIndex {
    ...
    private record Entry(Header header,Set<String> countries) {}
    ...
    public synchronized List<String> countries(String owner,String lifecycle){
        ...
        for(int start=0;start<changed.size();start+=500){
            jdbc.sql("""
                SELECT id,jsonb_build_object('country',payload->'country','options',
                ...
                .query((rs,n)->{
                    ...
                    return new Entry(current.get(rs.getObject("id",UUID.class)),Set.copyOf(countries));
                }).list().forEach(entry->entries.put(entry.header().id(),entry));
        }
        var countries=new HashSet<String>();for(var h:headers)countries.addAll(entries.get(h.id()).countries());
