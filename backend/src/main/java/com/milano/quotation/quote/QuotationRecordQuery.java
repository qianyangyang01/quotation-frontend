...
public class QuotationRecordQuery {
    ...
    public record Summary(long pending, long won, long lost, long total) {}
    ...
    public Result search(String owner, Filters filters, int page, int size) {
        ...
        if(filters.status()!=null && !filters.status().isBlank() && !Set.of("pending","won","lost").contains(filters.status())) throw AppException.unprocessable("报价状态不合法");
        ...
        if(filters.status()!=null && !filters.status().isBlank()) { where.append(" and status=:status");params.put("status",filters.status()); }
        ...
        var summary=jdbc.queryForObject("select count(*) total,count(*) filter(where status='pending') pending,count(*) filter(where status='won') won,count(*) filter(where status='lost') lost from quotation_record"+where,params,(rs,n)->new Summary(rs.getLong("pending"),rs.getLong("won"),rs.getLong("lost"),rs.getLong("total")));
