...
public class QuotationRecordQuery {
    ...
    public record Filters(String q, String status, String country, String category, LocalDate startDate, LocalDate endDate, String lifecycle) {
        public Filters(String q, String status, String country, String category, LocalDate startDate, LocalDate endDate) { this(q,status,country,category,startDate,endDate,"active"); }
    }
    ...
    public Result search(String owner, Filters filters, int page, int size) {
        ...
        var items=jdbc.query("select payload,version,lifecycle_state from quotation_record"+where+" order by created_at desc,id desc limit :limit offset :offset",params,(rs,n)->{var payload=(tools.jackson.databind.node.ObjectNode)mapper.readTree(rs.getString("payload"));payload.put("_version",rs.getLong("version"));payload.put("lifecycleState",rs.getString("lifecycle_state"));return (JsonNode)payload;});
