...
public class QuotationTemplateController {
    ...
    @PutMapping("/{id}") @Transactional ApiResponse<JsonNode> update(@PathVariable UUID id,@RequestBody ObjectNode body,Authentication auth){var row=owned(id,auth);if(!body.has("_version")||body.path("_version").asLong(-1)!=row.version)throw AppException.conflict("报价模板已被其他页面修改，请刷新后重试");var merged=(ObjectNode)row.payload.deepCopy();body.fields().forEachRemaining(entry->{if(!entry.getKey().equals("_version"))merged.set(entry.getKey(),entry.getValue());});var name=merged.path("name").asText(row.name).trim();if(name.isEmpty()||name.length()>120)throw AppException.unprocessable("模板名称不合法");row.name=name;row.payload=merged;row.updatedAt=Instant.now();templates.saveAndFlush(row);audit.record("template.update","quotation-template",id.toString(),"success",Map.of());return ApiResponse.ok(view(row));}
