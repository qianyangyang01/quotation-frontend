...
public class PurchaseWorkbookService {
    ...
    public Preview preview(MultipartFile file,String actor){
        ...
        try(var workbook=new XSSFWorkbook(file.getInputStream())){
            ...
            for(int sheetIndex=0;sheetIndex<workbook.getNumberOfSheets();sheetIndex++){
                ...
                var headers=values(sheet.getRow(0),64,formatter,evaluator);var schema=PurchaseWorkbookSchema.identify(headers,sheetName);sheetCount++;
                var pictures=pictures(sheet);
                for(int rowIndex=1;rowIndex<=sheet.getLastRowNum();rowIndex++){
                    int excelRow=rowIndex+1;var values=values(sheet.getRow(rowIndex),64,formatter,evaluator);
                    for(int column=schema.width();column<values.length;column++)if(!values[column].isBlank())throw AppException.unprocessable("工作表“"+sheetName+"”第"+excelRow+"行的重复尾列"+PurchaseWorkbookSchema.column(column)+"存在数据，请删除后重新上传");
                    ...
                    var mapped=rowMapper.map(sheetName,excelRow,Arrays.copyOf(values,schema.width()),schema);var payload=mapped.payload();
            ...
            summary.put("pending",pending).put("ignoredRows",ignored).put("generatedSku",0).put("productImages",productImages).put("physicalImages",physicalImages).put("skipped",0).put("sheetCount",sheetCount).put("errorCount",errorCount).put("blockingErrorCount",blockingErrorCount).put("warningCount",warningCount).put("canConfirm",blockingErrorCount==0&&duplicateSkus.isEmpty()).set("duplicateGroups",duplicateGroups).set("issues",allIssues);
    ...
    private static String[] values(Row row,int width,DataFormatter formatter,FormulaEvaluator evaluator){var out=new String[width];Arrays.fill(out,"");if(row==null)return out;for(int i=0;i<width;i++){var cell=row.getCell(i);if(cell!=null)out[i]=formatter.formatCellValue(cell,evaluator).trim();}return out;}
