...
public class AsyncPurchaseImportService {
    private static final long MAX_FILE=100L*1024*1024,MAX_LEGACY_ORIGINAL_FILE=1024L*1024*1024;public static final String JOB_TYPE="purchase-xlsx-async";
    ...
    public ImportJob create(MultipartFile file,String actor,String importMode,Long originalSizeBytes,Integer removedMediaCount){return create(file,actor,importMode,originalSizeBytes,removedMediaCount,PurchaseImportProfile.STANDARD);}
    public ImportJob create(MultipartFile file,String actor,String importMode,Long originalSizeBytes,Integer removedMediaCount,String importProfile){if(file.isEmpty()||file.getOriginalFilename()==null||!file.getOriginalFilename().toLowerCase(Locale.ROOT).endsWith(".xlsx"))throw AppException.unprocessable("请选择.xlsx格式的采购模板");if(!"text-only".equals(importMode))throw AppException.unprocessable("采购 Excel 仅支持无图数据导入");var profile=PurchaseImportProfile.normalize(importProfile);long originalBytes=originalSizeBytes==null?file.getSize():originalSizeBytes;int removed=removedMediaCount==null?0:removedMediaCount;long maxOriginal=PurchaseImportProfile.LEGACY_2026.equals(profile)?MAX_LEGACY_ORIGINAL_FILE:MAX_FILE;if(file.getSize()>MAX_FILE)throw AppException.unprocessable("去除图片后的采购数据文件不能超过100MB");if(originalBytes>maxOriginal)throw AppException.unprocessable(PurchaseImportProfile.LEGACY_2026.equals(profile)?"2026旧数据原文件不能超过1GB":"采购数据文件不能超过100MB");if(originalBytes<=0||removed<0||removed>200_000)throw AppException.unprocessable("无图数据文件统计信息不合法");var id=UUID.randomUUID();var key="purchase-import/"+id+"/source.xlsx";try{var hash=storage.putRawWithSha256(key,file.getInputStream(),file.getSize(),"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");var build=(java.util.function.Supplier<ImportJob>)()->{var now=Instant.now();var job=new ImportJob();job.id=id;job.jobType=JOB_TYPE;job.status="queued";job.phase="queued";job.requestedBy=actor;job.sourceName=safe(file.getOriginalFilename());job.sourceHash=hash;job.sourceObjectKey=key;job.payload=mapper.createObjectNode().put("sourceBytes",file.getSize()).put("uploadedBytes",file.getSize()).put("originalSizeBytes",originalBytes).put("removedMediaCount",removed).put("importMode","text-only").put("importProfile",profile);job.createdAt=now;job.updatedAt=now;job.heartbeatAt=now;PurchaseImportContinuationService.initialize(job);PurchaseImportBatchService.syncPayload(job);return jobs.save(job);};try{return transactions==null?build.get():transactions.execute(status->build.get());}catch(Exception db){storage.removeRaw(key);throw db;}}catch(Exception e){if(e instanceof AppException a)throw a;throw AppException.unprocessable("采购文件上传失败");}}
    public ImportJob confirm(UUID id,Map<String,DuplicateSelection> selections){
        java.util.function.Supplier<Confirmation> work=()->{
            ...
            if(PurchaseImportContinuationService.enabled(job)){if(PurchaseImportProfile.isLegacy(job))rows.keepLastFileDuplicate(id);else rows.markFileDuplicates(id);}
