...
public class PurchaseImportBatchService {
    ...
    @Transactional public void ready(UUID jobId){var job=jobs.findById(jobId).orElseThrow();if(continuation!=null)continuation.refresh(job);rows.markFileDuplicates(jobId);recount(job);job.status="ready";job.phase="ready";job.progressPercent=100;job.updatedAt=Instant.now();job.heartbeatAt=job.updatedAt;syncPayload(job);}
    ...
    @Transactional public void rollback(UUID jobId){if(continuation!=null)continuation.lockSource(jobs.findById(jobId).orElseThrow());var conflicts=jdbcImports.lockAndCountRollbackConflicts(jobId);if(conflicts>0)throw AppException.conflict("有 "+conflicts+" 条商品在导入后被修改，已阻止整批回滚");while(true){var ids=jdbcImports.nextRollbackIds(jobId);if(ids.isEmpty())break;jdbcImports.rollback(jobId,ids);jobs.heartbeat(jobId,AsyncPurchaseImportService.JOB_TYPE,Instant.now());}finishRollback(jobId);}
