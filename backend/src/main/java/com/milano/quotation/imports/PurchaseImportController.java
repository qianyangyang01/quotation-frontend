...
public class PurchaseImportController {
    ...
    ResponseEntity<ApiResponse<?>> createJob(@RequestPart("file")MultipartFile file,
                                              ...
                                              @RequestParam(defaultValue="standard")String importProfile,
                                              @RequestParam(required=false)Long originalSizeBytes,
                                              @RequestParam(defaultValue="0")Integer removedMediaCount,
                                              Authentication auth){var job=asyncImports.create(file,account(auth),importMode,originalSizeBytes,removedMediaCount,importProfile);audit.record("purchase.async-import-create","purchase-import",job.id.toString(),"success",Map.of("file",job.sourceName,"uploadedSize",file.getSize(),"originalSize",job.payload.path("originalSizeBytes").asLong(),"removedMediaCount",job.payload.path("removedMediaCount").asInt(),"importMode","text-only","importProfile",job.payload.path("importProfile").asText(PurchaseImportProfile.STANDARD)));return ResponseEntity.accepted().body(ApiResponse.ok(asyncImports.view(job.id)));}
