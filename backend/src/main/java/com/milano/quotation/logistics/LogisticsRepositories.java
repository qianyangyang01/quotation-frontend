...
interface LogisticsChannelRepository extends JpaRepository<LogisticsChannelEntity,UUID>{boolean existsByCodeIgnoreCase(String code);Optional<LogisticsChannelEntity> findByCodeIgnoreCase(String code);long countByProviderId(UUID providerId);long countByCurrentVersionIdIsNotNull();List<LogisticsChannelEntity> findAllByOrderByUpdatedAtDesc();}
