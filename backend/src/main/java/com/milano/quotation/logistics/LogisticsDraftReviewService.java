...
public class LogisticsDraftReviewService {
    ...
    @Transactional public ObjectNode patch(UUID versionId,ObjectNode input,String actor){
        ...
        var changes=input.path("changes");if(!changes.isArray()||changes.isEmpty()||changes.size()>5000)throw AppException.unprocessable("请选择1至5000条价格修正");
        var payload=(ObjectNode)stored.path("payload").deepCopy();var rows=(ArrayNode)payload.withArray("rows");var byKey=new LinkedHashMap<String,ObjectNode>();
        ...
        var errors=count(preserved,"error");var pending=false;for(var row:rows)if(!row.path("pendingReason").asText().isBlank())pending=true;
        payload.put("errors",errors).put("warnings",count(preserved,"warning")).put("validRows",rows.size()).put("quoteReady",errors==0&&!pending&&"known".equals(payload.path("templateStatus").asText())).put("contentHash",LogisticsDatasetService.hash(rows.toString()));
        ...
        var history=payload.withArray("correctionHistory");var event=history.addObject().put("editedBy",actor).put("editedAt",editedAt);event.set("changes",audit);
    ...
    ObjectNode load(UUID id,boolean lock){var sql="select jsonb_build_object('id',v.id,'channelId',v.channel_id,'versionNumber',v.version_number,'status',v.status,'payload',v.payload,'fingerprint',v.rows_fingerprint)::text from logistics_version v where v.id=:id"+(lock?" for update":"");
        ...
        var result=(ObjectNode)row.path("payload").deepCopy();
        result.put("pricingReady",result.path("quoteReady").asBoolean(false));
        result.put("id",row.path("id").asText()).put("channelId",row.path("channelId").asText()).put("versionNumber",row.path("versionNumber").asInt()).put("status",row.path("status").asText()).put("fingerprint",row.path("fingerprint").asText()).put("quoteReady",false);return result;}
    private static boolean isEditableIssue(JsonNode issue){var code=issue.path("code").asText();if(code.startsWith("WEIGHT_"))return true;var field=issue.path("field").asText();return Set.of("重量区间","计费价格","区域/重量区间","重量连续性").contains(field);}
