...
public class LogisticsBillingEngine {
    public static final String VERSION="logistics-billing-v9";
    ...
    public List<String> unsupported(JsonNode rows) {
        ...
        for(var source:rows){
            ...
            if(!LogisticsStepPricing.supported(row))reasons.add("计费进位参数无效");
    ...
    public ObjectNode calculate(JsonNode rows,JsonNode input) {
        ...
        for(var source:rows){int current=index++;
            var row=LogisticsRoundingNotes.apply(source);
            ...
            else charge=LogisticsStepPricing.charged(row,charge);
        ...
        if(matches.size()>1){var totals=new HashSet<String>();for(var match:matches)totals.add(priceKey(LogisticsRoundingNotes.apply(rows.get(match.path("rowIndex").asInt()))));if(totals.size()>1)throw AppException.unprocessable("存在多个匹配价格，禁止自动选择最低价");}
    ...
    private static String priceKey(JsonNode row){
        var values=new ArrayList<String>();values.add(model(row));values.add(LogisticsStepPricing.key(row));values.add(minimum(row).stripTrailingZeros().toPlainString());
