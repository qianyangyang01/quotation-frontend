...
@Entity @Table(name="logistics_provider") class LogisticsProviderEntity{
    @Id UUID id;@Column(nullable=false,length=64)String code;@JdbcTypeCode(SqlTypes.JSON)@Column(nullable=false,columnDefinition="jsonb")JsonNode payload;@Version long version;@Column(name="created_at",nullable=false)Instant createdAt;@Column(name="updated_at",nullable=false)Instant updatedAt;protected LogisticsProviderEntity(){}
}
@Entity @Table(name="logistics_channel") class LogisticsChannelEntity{
    @Id UUID id;@Column(name="provider_id",nullable=false)UUID providerId;@Column(nullable=false,length=96)String code;@Column(name="rule_id",nullable=false,unique=true)int ruleId;@Column(name="current_version_id")UUID currentVersionId;@Column(name="archived_at")Instant archivedAt;@Column(name="archived_by",length=120)String archivedBy;@Column(name="archive_reason",length=500)String archiveReason;@JdbcTypeCode(SqlTypes.JSON)@Column(nullable=false,columnDefinition="jsonb")JsonNode payload;@Version long version;@Column(name="created_at",nullable=false)Instant createdAt;@Column(name="updated_at",nullable=false)Instant updatedAt;protected LogisticsChannelEntity(){}
}
