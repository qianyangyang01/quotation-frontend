...
for (let index = 0; index < users; index += 1) {
  ...
  await session.login()
  session.role = !roleMix || index < 20 ? 'employee' : index < 48 ? 'purchase' : 'logistics'
...
const progressTimer=setInterval(()=>process.stdout.write(`${JSON.stringify({progress:true,measuredOperations:total,failures:failures.length,elapsedSeconds:Math.round((Date.now()-Date.parse(startedAt))/1000)})}\n`),60000)
...
if (roleMix) queryNames.push('purchase-role-search', 'purchase-role-detail', 'purchase-role-stats', 'logistics-role-channels', 'logistics-role-providers', 'logistics-role-versions')
const queryP95Ms=500,queryP99Ms=1000,saveP95Ms=1000,saveP99Ms=2000,maxErrorRate=0.001
const thresholdFailures = queryNames.filter(name => (operations[name]?.p95Ms ?? Infinity) > queryP95Ms || (operations[name]?.p99Ms ?? Infinity) > queryP99Ms)
for (const name of ['quotation-save-single', 'quotation-save-bundle']) {
  if (!readOnly && ((operations[name]?.p95Ms ?? Infinity) > saveP95Ms || (operations[name]?.p99Ms ?? Infinity) > saveP99Ms)) thresholdFailures.push(name)
}
if (failureRate >= maxErrorRate) thresholdFailures.push('error-rate')
if (roleMix) for (const name of ['purchase-create-verify', 'logistics-provider-add']) {
  if ((operations[name]?.p95Ms ?? Infinity) > saveP95Ms || (operations[name]?.p99Ms ?? Infinity) > saveP99Ms) thresholdFailures.push(name)
}
const report = {
  ...
  thresholds: { queryP95Ms,queryP99Ms,saveP95Ms,saveP99Ms,maxErrorRate,passed: thresholdFailures.length === 0, failed: thresholdFailures },
