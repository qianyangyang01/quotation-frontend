...
SELECT 'unrelated-logistics',count(*),md5(coalesce(string_agg(md5(c.id::text||c.payload::text||coalesce(v.payload::text,'')),'' ORDER BY c.id),'')) FROM logistics_channel c JOIN logistics_provider p ON p.id=c.provider_id LEFT JOIN logistics_version v ON v.id=c.current_version_id WHERE p.code NOT LIKE 'LIMP-%';
